...
-
		...
		Conta contaUpdate = manager.find(Conta.class, 9);
		...
		// Apos a execucao, verifique na console se uma cláusula 
		// Update foi gerada.

		// Repita e execucao e repare que da segunda vez, nenhum update vai 
		// ser executado, isso pq a entidade representa exatamente os mesmos
		...
		contaJamesDetached.setId(10);
		contaJamesDetached.setBanco("007 - BANCO DO JAMES");
		contaJamesDetached.setNumero("7777-0");
		contaJamesDetached.setAgencia("0000");
		contaJamesDetached.setTitular("James Bond");
		...
		*/
		...
		contaJamesDetached = manager.find(Conta.class, 10);
		manager.remove(contaJamesDetached);
		manager.getTransaction().commit();
